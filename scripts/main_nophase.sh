...
#SBATCH --chdir=./Stylus
#SBATCH --exclusive
#SBATCH --output=./Stylus/logs/R_all_nophase.9-%j-%x.out
set +x
...
do
    ...
    do
        echo $cont $sty
        torchrun --standalone --nproc_per_node=4 ./Stylus/main_nophase.py --gamma 0.75 --cnt "$data_dir/content/$cont" --sty "$data_dir/timbre/$sty" --output_path $out_dir
    done
done
