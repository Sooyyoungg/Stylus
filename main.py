...
def feat_merge(opt, cnt_feats, sty_feats, start_step=0, num_steps=50):
    feat_maps = [{'config': {'gamma':opt.gamma, 'alpha':opt.alpha, 'T':opt.T, 'timestep':_}} for _ in range(num_steps)]
    for i in range(len(feat_maps)):
        if i < (num_steps - start_step): continue
        cnt_feat, sty_feat = cnt_feats[i], sty_feats[i]
        #print(f"sty: {len(sty_feat.keys())}, cnt:{len(cnt_feat.keys())}")

        for sty_key, cnt_key in zip(sty_feat.keys(), cnt_feat.keys()):
            if cnt_key[-1] == 'q':
                feat_maps[i][cnt_key] = cnt_feat[cnt_key]
            if cnt_key[-1] == 'k' or cnt_key[-1] == 'v':
                feat_maps[i][f'content_{cnt_key}'] = cnt_feat[cnt_key]
            if sty_key[-1] == 'k' or sty_key[-1] == 'v':
                feat_maps[i][f'style_{sty_key}'] = sty_feat[sty_key]
    return feat_maps
...
def main():
    ...
    parser.add_argument('--T', type=float, default=1.5, help='attention temperature scaling for style injection')
    parser.add_argument('--gamma', type=float, default=0.75, help='query preservation hyperparameter')
    parser.add_argument('--alpha', type=float, default=0.75, help='query preservation hyperparameter')
    ...
    parser.add_argument('--output_path', type=str, default='output', help='output')
    parser.add_argument("--without_init_adain", action='store_true')
    parser.add_argument("--without_key_feature", action='store_true')
    parser.add_argument("--without_value_feature", action='store_true')
    ...
    if is_main_process:
        ...
        os.makedirs(os.path.join(opt.output_path, 'audios_griffin'), exist_ok=True)
        os.makedirs(os.path.join(opt.output_path, 'audios_phase'), exist_ok=True)
    ...
    save_feature_timesteps = opt.save_feat_steps
    start_step = ddim_inversion_steps-1
    assert ddim_inversion_steps == save_feature_timesteps
    ...
    for n_sample, (sty_name, cnt_name) in enumerate(pairs_for_this_rank):
        ...
        feat_maps = [{'config': {'gamma': opt.gamma, 'alpha':opt.alpha, 'T': opt.T}} for _ in range(save_feature_timesteps)]
        ...
        with torch.no_grad():
            with precision_scope("cuda"):
                with raw_model.ema_scope():
                    ...
                    x_sample_img_np = cmap(x_sample_img_np)
                    ...
                    audio_path = os.path.join(opt.output_path, 'audios_griffin', output_name) + '.wav'
                    ...
                    audio_path = os.path.join(opt.output_path, 'audios_phase', output_name) + '.wav'
